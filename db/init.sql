create table rezeptliste (Name text not null, IdRezept int auto_increment, Favorit tinyint(1) not null, Beschreibung text not null, Zeit int not null, BildUrl text not null, constraint rezeptliste_IdRezept_uindex unique (IdRezept) );
alter table rezeptliste add primary key (IdRezept);

create table kategorien (IdKategorie int auto_increment, Kategorie char(30) not null, fkRezept int not null, constraint kategorien_IdKategorie_uindex unique (IdKategorie), constraint kategorien_rezeptliste_IdRezept_fk foreign key (fkRezept) references rezeptliste (IdRezept) );
alter table kategorien add primary key (IdKategorie);

create table zutaten (IdZutat int auto_increment, Zutat char(50) not null, Menge char(50) not null, fkRezept int not null, constraint zutaten_IdZutat_uindex unique (IdZutat), constraint zutaten_rezeptliste_IdRezept_fk foreign key (fkRezept) references rezeptliste (IdRezept) );
alter table zutaten add primary key (IdZutat);
